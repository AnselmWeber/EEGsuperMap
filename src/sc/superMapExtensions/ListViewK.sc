ListViewK : ItemViewBase {
	...
	*new { arg parent, bounds, key;
	...
	init {
		...
		OSCFunc({
			AppClock.sched(0.05, {
				var rand_selection = {(0..this.items.size - 1).choose} ! this.items.size;
				this.selection_(rand_selection)
			});
		...
		OSCFunc({|msg|
			...
			AppClock.sched(0.00, {
				...
				this.selection_(load_selection);
			});
		}, '/util/reload');
	}
