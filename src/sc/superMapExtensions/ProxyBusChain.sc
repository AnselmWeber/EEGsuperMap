...
ProxyDictChain {
	...
	*addSource { |srcName, source|
		var dict = this.atSrcDict(srcName);
		var srcFunc, paramNames, isFilter = false, bus;
		...
		if (source.notNil) {
			...
			srcFunc = if (source.isKindOf(Association)) {
				isFilter = [\filter, \filterIn, \split].includes(source.key);
				source.value
			} { source };
	...
	*checkSourceDictAt { |srcname|
		...
		paramNames.do { |name|
			...
			spec = spec ?? { ProxyDictChain.getSpec(name) };
