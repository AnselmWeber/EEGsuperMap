...
+ AbstractPlayControl {
	...
	*initClass {
		...
		buildMethods = (
			...
			split: #{ | func, proxy, channelOffset = 0, index |
				var ok, ugen;
				if(proxy.isNeutral) {
					ugen = func.value(Silent.ar);
					ok = proxy.initBus(ugen.rate, ugen.numChannels + channelOffset);
				...
				{ | out |
					var env, bus, buf, sig, rate, phase, busName, ctl = NamedControl.kr("wet" ++ (index ? 0), 1.0), outsynth;
					//bus = Archive.global.at(\testbus)[\tebu0];
					if(proxy.rate === 'audio') {
						["SPLITKEY:  " ++ proxy.key].postln;
						busName = func.argNames[func.argNames.size - 1] ++ proxy.key;

						//buf = Buffer.alloc(Server.default, 1024, 1, bufnum: 50);

						/*
						if(Archive.global.at(\bus_data)[busName].isNil, {
						bus = Bus.audio(Server.local, 1);
						Archive.global.at(\bus_data)[busName] = bus;
						}, {
						//bus = Archive.global.at(\bus_data)[busName];
						bus = Bus.audio(Server.local, 1);
						});
						*/
						//bus = Archive.global.at(\bus_data)[busName];

						//Archive.global.at(\bus_data)[busName] = Bus.audio(Server.local, 1);
						busName.postln;
						bus = BusDictManager().getOrSet(busName, Bus.audio(Server.local, 1));
						//bus = Bus.audio(Server.local, 1);
						["splitbusname: " ++ busName ++ " " ++ bus.index ++ "key: " ++  proxy.key].postln;
						...
						// numChannels überprüfen!!
						//bus = Bus.audio(s, 1);

						busName.postln;
						outsynth.clear;
						Archive.global.at(busName.asSymbol).clear;
						Archive.global.put(busName.asSymbol, SynthDef.wrap(func, nil, [In.ar(out.asArray[0], proxy.numChannels)]););

						XOut.ar(bus, env, Archive.global.at(busName.asSymbol));
						XOut.ar(out, env, Archive.global.at(busName.asSymbol));
						*/


						// this was working but produced too many synths!

						XOut.ar(bus, env, SynthDef.wrap(func, nil, [In.ar(out.asArray[0], proxy.numChannels)]));
						XOut.ar(out, env, SynthDef.wrap(func, nil, [In.ar(out, proxy.numChannels)]));
